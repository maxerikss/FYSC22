...
def mass_diff(data: np.ndarray) -> np.ndarray:
	...
	data1 = data[idx1]
	data2 = data[idx2]
	mass_diff_total = (1e6+data2[:,3] - data1[:,3]) * 0.0009315 # MeV / u
...
def plot(X, Y, error):
	...
	ax.scatter(X/scale, Y, c='fuchsia', s=16, zorder=1, label='Data points')
	ax.errorbar(X/scale, Y, yerr=error, ls='None', zorder=3, label='Error')
